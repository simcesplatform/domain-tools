...
class CsvFilePriceStateSource():
    ...
    def getNextEpochData(self) -> PriceForecastState:
        ...
        try:
            row = next( self._csv )
            # ed_rwo is the row I would like to edit it, remove some items, etc
            ed_row = row
        ...
        # that in the ed_row, all other data are removed and we only have price abd timestamp data 
        no_interval = int(len(ed_row)/2)
        timedata_list = []
        pricedata_list = []
